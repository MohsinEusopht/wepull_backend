...
module.exports = {
    ...
    getQuickbooksExpenseByAccount: (account_id, company_id) => {
        ...
        return new Promise((resolov, reject) => {
            pool.query(
                `SELECT e.expense_id,e.created_at,e.updated_at,e.txn_date,e.currency,e.payment_type,e.description,e.account_number,a.name as 'account_name',e.credit,e.entity_ref_name,e.entity_ref_type,e.total_amount,c.company_name from expenses e left join accounts a on e.account_number=a.accountID and e.company_id=a.company_id join companies c ON e.company_id = c.id where e.company_id = ? and e.account_number = ? ORDER BY e.created_at ASC`, [company_id, account_id],
                (error, results, fields) => {
                    if (error) {
                        return reject(error);
                    }
    ...
    getXeroExpenseByCategoryAndVendor: (company_id, category_id, vendor_id) => {
        return new Promise((resolov, reject) => {
            pool.query(
                `SELECT e.expense_id,e.created_at,e.updated_at,e.txn_date,e.currency,e.payment_type,e.description,e.account_number,a.name as 'account_name',e.credit,e.entity_ref_name,e.entity_ref_type,e.total_amount,e.is_paid,e.payment_ref_number, e.paid_amount, e.payment_date,c.company_name from expenses e left join accounts a on e.account_number=a.code and e.company_id=a.company_id join companies c ON e.company_id = c.id where e.company_id = ? and e.department_id = ? and e.entity_ref_number = ? ORDER BY e.created_at ASC`, [company_id, category_id, vendor_id],
                (error, results, fields) => {
                    if (error) {
                        return reject(error);
                    }
